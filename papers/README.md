...
| [multi-stage-solicitation.md](multi-stage-solicitation.md) | Working draft v0.2 | The more novel companion: the design space of chained elicitation, organized by representation (point outputs vs probabilistic outputs) and by which chains the theory supports. The two ways to score a downstream stage (locally vs top-level conversion) and their log-score equivalence; the anchor requirement and the unanchored failure case. Then what has been built, fact-checked against `microprediction/rediz` and `microconventions`: 225-sample streams, z1~ PIT streams, z2~/z3~ Morton z-curve copula streams, monteprediction, MidOne. Cites the algebra for single-stage theory. |
