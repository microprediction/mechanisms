...
| [combining-linear-fee-market-makers.md](combining-linear-fee-market-makers.md) | Working draft v0.1 | "Combining Linear-Fee Market Makers". A proportional fee, conjugated, is exactly a bid-ask spread; combining fee-bearing cost-function makers is an infimal convolution solved by one monotone clearing-price root-find with lasso-sparse fills; the aggregate supply curve is a consolidated limit order book, and each maker may quote its own fee with routing as the discipline. Companion to the research note [proportional-fees-and-the-order-book.md](../research/proportional-fees-and-the-order-book.md); implemented in [`mechanisms/fee_routing.py`](../mechanisms/fee_routing.py). |
