"""Simulation: the Bayesian Truth Serum rewards truthfulness without ground truth.
...
reporting is a Bayesian Nash equilibrium. Here a population of truthful respondents
draws private signals from a shared truth; one strategic respondent always picks
the popular answer with an overconfident prediction. The truthful crowd out-scores
...
def simulate(n_truthful=40, rounds=400, seed=0):
    ...
    for _ in range(rounds):
        ...
        truthful_totals += scores[:n_truthful]
        strategist_total += scores[n_truthful]

    mean_truthful = truthful_totals.mean() / rounds
    mean_strategist = strategist_total / rounds

    section("Bayesian Truth Serum — mean score per round (higher is better)")
    labeled_bars(
        [
            ("truthful crowd (avg)", mean_truthful),
            ("strategist", mean_strategist),
        ],
        vmax=max(mean_truthful, mean_strategist, 1e-9),
        fmt="{:+.3f}",
    )
    verdict = "truthful crowd wins" if mean_truthful > mean_strategist else "strategist wins"
