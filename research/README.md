...
| [predictors-as-markets.md](predictors-as-markets.md) | Which predictors are markets: coherence is a chord condition (not convexity), rational flow trades the biconjugate (contact-set landing, gap pass-through, the excluded range showing as a block at one price), a fee of at least the chord excursion restores no-arbitrage (generalizing the linear-fee lemma to non-convex costs), and a deep quadratic co-quoter is Moreau smoothing; adaptive/path-dependent learners map to the adaptive-liquidity trilemma | Chord condition, contact/pass-through identities, generalized fee lemma and Moreau-by-merge not found (nearest: canonical concave CFMM trading functions, Angeris et al. 2023; Guasoni/GRS transaction-cost no-arbitrage; Frongillo-Waggoner 2018 fee repair); rendered as the working paper [non-convex-market-makers.md](../papers/non-convex-market-makers.md); implemented in `nonconvex_maker.py` |
| [serial-markets-and-belief-propagation.md](serial-markets-and-belief-propagation.md) | Serial composition is the chain rule: one market per factor, opening when conditioning freezes, settling in cascade; the market step is the Kalman update (exact), trades are BP messages on trees (exact), loopy inconsistency is a sure-arbitrage certificate; parallel/serial are the two semiring operations of inference, with Gaussian exactness because sum-product and min-sum agree there | Heavily occupied territory, credited (Hanson modularity; Pennock-Wellman 1996 market-Bayes economies with arbitrageur agents; junction-tree pricing in production, Sun et al. 2012); unoccupied residue: the per-factor serial architecture, market-implemented filtering, loopy-arbitrage framing; the serial material now sits in [clamped-markets.md](../papers/clamped-markets.md); implemented in `serial_markets.py` |
