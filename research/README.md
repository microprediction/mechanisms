...
| [scoring-rules-as-market-making.md](scoring-rules-as-market-making.md) | **Proper scoring rules *are* market making** — the market-making reading of Savage's characterisation: one convex generator gives a proper rule (hyperplane), a cost-function market maker (conjugate), prices (gradient) and trader profit (Bregman divergence); CFMM equivalence and the web out to elicitation, peer prediction, local scoring | `scoring_rules`, `lmsr`, `cmm`, `amm` |
